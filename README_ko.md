...
<ul style="font-size: 14px; line-height: 1.8; color: #444; margin: 10px 0;">
  ...
  <li><strong>모바일 APP 관리 백오피스</strong>: uni-app 3 기반 크로스 플랫폼 모바일 관리 백오피스로, 하나의 코드베이스를 H5, WeChat 미니 프로그램, 네이티브 App으로 컴파일합니다. PC(WEB) 클라이언트와 동일한 백엔드 API(<code>/admin-api</code>)를 공유하여, 운영 및 관리 담당자가 모바일에서 언제 어디서나 플랫폼을 제어할 수 있습니다. 핵심 탭은 <strong>디바이스</strong>(직접 연결 카메라, GB28181, NVR 통합 목록 및 채널 탐색, 디바이스 상세에서 원터치 실시간 미리보기), <strong>스트림 전달</strong>(작업 생성, 시작/중지, 클러스터 노드 상태 및 다중 스트림 URL 조회), <strong>알고리즘</strong>(실시간/스냅샷 알고리즘 작업 목록, 시작/중지 제어 및 검출 통계), <strong>경보</strong>(경보 이벤트 검색, 스냅샷 미리보기 및 경보 녹화 VOD 재생), <strong>모델</strong>(모델 목록 및 배포 상태), <strong>추론</strong>(모바일 이미지 추론 워크벤치: 모델 선택, 이미지 업로드, 결과 확인), <strong>훈련</strong>(훈련 작업 진행률 모니터링 및 원터치 중지), <strong>내 정보</strong>(개인 프로필, 계정 보안, 멀티 테넌트 전환 및 앱 설정)를 포함합니다. H5는 Jessibuca 저지연 플레이어를 통합하여 FLV/HLS 실시간 스트림 및 경보 녹화 VOD를 지원하며, OAuth2 듀얼 토큰 인증과 Pinia 상태 영속화로 세션을 자동 갱신합니다 — 클라우드-엣지-디바이스 지능형 제어를 스마트폰과 미니 프로그램까지 확장합니다.</li>
</ul>
...
<td style="padding: 15px; border: 1px solid #e0e0e0; line-height: 1.8; color: #444;">
  <ul style="margin: 5px 0; padding-left: 20px;">
    <li><strong>크로스 플랫폼 프레임워크</strong>: uni-app 3 + Vue 3 + TypeScript + Vite 기반, 하나의 코드베이스로 H5, WeChat 미니 프로그램, 네이티브 App 컴파일</li>
    <li><strong>API 재사용</strong>: WEB 모듈과 <code>/admin-api</code> 백엔드 공유, OAuth2 듀얼 토큰 인증, 멀티 테넌트 전환 지원</li>
    <li><strong>디바이스 관리</strong>: 직접 연결 카메라, GB28181, NVR 통합 목록, 온라인 상태 및 채널 탐색, 디바이스 상세 실시간 미리보기(H5 Jessibuca 저지연 플레이어 통합)</li>
