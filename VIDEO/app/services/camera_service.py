...
def probe_onvif_http_endpoint(ip: str, port: int, connect_timeout: float) -> bool:
    """探测 http://ip:port/onvif/device_service 是否为 ONVIF SOAP 端点（供扫描器等复用）。"""
    return _probe_onvif_endpoint(ip, port, connect_timeout)
...
def _sync_existing_camera_from_onvif_scan(
    camera: Device,
    camera_info: dict,
    username: str,
    password: str,
    ...
    """同一 MAC 再次被扫描发现：刷新 IP、端口、RTSP 源与 ONVIF 元数据（保留设备 ID）。"""
    from app.services.onvif_scan_service import is_ip_blacklisted

    if is_ip_blacklisted(probe_ip):
        logger.info('黑名单 IP，跳过 ONVIF 同步: %s', probe_ip)
        return

    manufacturer = camera_info.get('manufacturer', '').strip() if camera_info.get('manufacturer') else ''
    model = camera_info.get('model', '').strip() if camera_info.get('model') else ''
    if not manufacturer:
        manufacturer = (camera.manufacturer or '').strip() or 'EasyAIoT'
    if not model:
        model = (camera.model or '').strip() or 'Camera-EasyAIoT'

    new_ip = (camera_info.get('ip') or '').strip() or probe_ip
    raw_port = camera_info.get('port', connect_port)
    try:
    ...
    new_source = camera_info.get('source')
    if new_source:
        camera.source = new_source
        camera.rtmp_stream, camera.http_stream, camera.ai_rtmp_stream, camera.ai_http_stream = _generate_stream_urls(
            new_source, camera.id
        )
    ...
    camera.model = model
    camera.firmware_version = camera_info.get('firmware_version')
    camera.serial_number = camera_info.get('serial_number')
    camera.hardware_id = camera_info.get('hardware_id')
    camera.support_move = camera_info.get('support_move', False)
    camera.support_zoom = camera_info.get('support_zoom', False)

    db.session.commit()
    _onvif_cameras.pop(camera.id, None)
    _monitor.update(camera.id, camera.ip, default_online=_is_custom_camera(camera))
    logger.info(
        '设备 %s ONVIF 扫描同步完成（同 MAC），当前 IP: %s ONVIF端口: %s',
        camera.id, camera.ip, new_port,
    )

    try:
        from app.services.stream_forward_service import ensure_device_stream_forward_task
        ensure_device_stream_forward_task(camera.id)
    except Exception as e:
        logger.warning('扫描同步后检查推流转发任务失败: %s', e)


def register_camera_by_onvif_with_credentials(
    ip: str, port: int, username: str, password: str,
) -> tuple[str, str]:
    """使用指定用户名、密码通过 ONVIF 注册或同步设备（批量扫描调用）。

    Returns:
        (device_id, outcome)：``created`` | ``updated`` | ``duplicate_rtmp_skip`` | ``blacklisted_skip``。
    """
    if not ip or not ip.strip():
        raise ValueError('摄像头IP地址不能为空')
    if not port or port <= 0:
        raise ValueError('摄像头端口必须大于0')
    if password is None:
        raise ValueError('摄像头密码不能为 None')

    from app.services.onvif_scan_service import is_ip_blacklisted

    if is_ip_blacklisted(ip):
        return ip, 'blacklisted_skip'

    temp_id = 'temp_' + str(time.time_ns())
    try:
        onvif_cam = _create_onvif_camera(temp_id, ip, port, username, password)
    except Exception as e:
        raise RuntimeError(f'无法连接到设备 {ip}:{port}：{str(e)}')
    ...
    if mac:
        existing_camera = Device.query.filter_by(mac=mac).first()
        if existing_camera:
            if existing_camera.source and existing_camera.source.strip().lower().startswith('rtmp://'):
                logger.debug(
                    '同 MAC 设备 %s 为 RTMP 源，跳过 ONVIF 同步',
                    existing_camera.id,
                )
                return existing_camera.id, 'duplicate_rtmp_skip'
            _sync_existing_camera_from_onvif_scan(
                existing_camera, camera_info, username, password, port, ip,
            )
            return existing_camera.id, 'updated'
    ...
    camera = Device(
        ...
        ai_http_stream=ai_http_stream,
        stream=None,
        ip=(camera_info.get('ip') or '').strip() or ip,
        port=camera_info.get('port', port),
        username=username,
        password=password,
        mac=camera_info.get('mac'),
    ...
    try:
        db.session.commit()
        _monitor.update(camera.id, camera.ip)
        logger.info(f'设备 {device_id} 通过 ONVIF 注册成功，IP: {camera.ip}, 用户名: {username}')

        try:
            from app.services.snap_space_service import create_snap_space_for_device
            create_snap_space_for_device(device_id, camera.name)
        except Exception as e:
            logger.warning(f'为设备 {device_id} 创建抓拍空间失败: {str(e)}')

        try:
            from app.services.record_space_service import create_record_space_for_device
            create_record_space_for_device(device_id, camera.name)
        except Exception as e:
            logger.warning(f'为设备 {device_id} 创建监控录像空间失败: {str(e)}')

        try:
            from app.services.stream_forward_service import ensure_device_stream_forward_task
            ensure_device_stream_forward_task(device_id)
        except Exception as e:
            logger.warning(f'为设备 {device_id} 自动创建推流转发任务失败: {str(e)}')
