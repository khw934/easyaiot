...
class OnvifPasswordLibrary(db.Model):
    """ONVIF 扫描撞库用密码库（用户名/密码对列表）"""
    __tablename__ = 'onvif_password_library'

    id = db.Column(db.Integer, primary_key=True, autoincrement=True)
    name = db.Column(db.String(255), nullable=False)
    lib_code = db.Column(db.String(128), nullable=False, unique=True, comment='唯一编码')
    description = db.Column(db.String(500), nullable=True)
    credentials_json = db.Column(db.Text, nullable=False, comment='JSON: [{"username":"admin","password":"xxx"}, ...]')
    created_at = db.Column(db.DateTime, default=lambda: datetime.utcnow())
    updated_at = db.Column(db.DateTime, default=lambda: datetime.utcnow(), onupdate=lambda: datetime.utcnow())

    tasks = db.relationship('OnvifScanTask', backref='password_library', lazy=True)
    ...
    def to_dict(self):
        return {
            ...
            'description': self.description,
            'credential_count': len(self._parse_credentials()),
            'created_at': self.created_at.isoformat() if self.created_at else None,
            'updated_at': self.updated_at.isoformat() if self.updated_at else None,
        }
    ...
    def to_dict_with_secrets(self):
        d = self.to_dict()
        d['credentials'] = self._parse_credentials()
        return d

    def _parse_credentials(self):
        import json
        try:
            data = json.loads(self.credentials_json or '[]')
...
class OnvifScanTask(db.Model):
    """ONVIF 分布式网段扫描任务实例"""
    __tablename__ = 'onvif_scan_task'

    id = db.Column(db.Integer, primary_key=True, autoincrement=True)
    task_code = db.Column(db.String(128), nullable=False, unique=True)
    task_name = db.Column(db.String(255), nullable=False)
    cidrs_json = db.Column(db.Text, nullable=False, comment='JSON: ["192.168.1.0/24", ...]')
    password_library_id = db.Column(db.Integer, db.ForeignKey('onvif_password_library.id', ondelete='SET NULL'), nullable=True)

    quick_scan_ports_json = db.Column(db.Text, nullable=False, default='[80,554]', comment='快速端口探测列表')
    quick_scan_timeout_ms = db.Column(db.Integer, nullable=False, default=200)
    ws_discovery_timeout_ms = db.Column(db.Integer, nullable=False, default=800)
    onvif_http_probe_timeout = db.Column(db.Float, nullable=False, default=0.4)

    max_hosts_per_cycle = db.Column(db.Integer, nullable=False, default=1024)
    max_total_hosts = db.Column(db.Integer, nullable=False, default=65536, comment='参与轮询的地址空间上限（防 /8 爆内存）')
    scan_interval_sec = db.Column(db.Integer, nullable=False, default=120)

    is_enabled = db.Column(db.Boolean, default=False, nullable=False)
    auto_register = db.Column(db.Boolean, default=True, nullable=False, comment='认证成功后是否自动入库')

    assigned_worker_id = db.Column(db.String(128), nullable=True, comment='仅该 worker 执行；空则任意 worker 可抢')
    claim_worker_id = db.Column(db.String(128), nullable=True)
    claim_lease_until = db.Column(db.DateTime, nullable=True)

    scan_cursor = db.Column(db.Integer, nullable=False, default=0, comment='扁平化 IP 列表上的游标')
    last_stats_json = db.Column(db.Text, nullable=True)
    last_error = db.Column(db.String(1000), nullable=True)
    last_cycle_at = db.Column(db.DateTime, nullable=True)

    description = db.Column(db.String(500), nullable=True)
    created_at = db.Column(db.DateTime, default=lambda: datetime.utcnow())
    updated_at = db.Column(db.DateTime, default=lambda: datetime.utcnow(), onupdate=lambda: datetime.utcnow())

    skip_entries = db.relationship('OnvifScanSkipEntry', backref='task', lazy=True, cascade='all, delete-orphan')

    def to_dict(self):
        import json
        cidrs = []
        try:
            cidrs = json.loads(self.cidrs_json or '[]')
        except Exception:
            pass
        ports = [80, 554]
        try:
            ports = json.loads(self.quick_scan_ports_json or '[80,554]')
        except Exception:
            pass
        return {
            'id': self.id,
            ...
            'task_name': self.task_name,
            'cidrs': cidrs,
            'password_library_id': self.password_library_id,
            'quick_scan_ports': ports,
            'quick_scan_timeout_ms': self.quick_scan_timeout_ms,
            'ws_discovery_timeout_ms': self.ws_discovery_timeout_ms,
            'onvif_http_probe_timeout': self.onvif_http_probe_timeout,
            'max_hosts_per_cycle': self.max_hosts_per_cycle,
            'max_total_hosts': self.max_total_hosts,
            'scan_interval_sec': self.scan_interval_sec,
            'is_enabled': self.is_enabled,
            'auto_register': self.auto_register,
            'assigned_worker_id': self.assigned_worker_id,
            'claim_worker_id': self.claim_worker_id,
            'claim_lease_until': self.claim_lease_until.isoformat() if self.claim_lease_until else None,
            'scan_cursor': self.scan_cursor,
            'last_stats': json.loads(self.last_stats_json) if self.last_stats_json else None,
...
class OnvifScanSkipEntry(db.Model):
    """端口存活但无 ONVIF 等：记入跳过名单，下轮不再浪费探测，除非手动放行"""
    __tablename__ = 'onvif_scan_skip_entry'

    id = db.Column(db.Integer, primary_key=True, autoincrement=True)
    task_id = db.Column(db.Integer, db.ForeignKey('onvif_scan_task.id', ondelete='CASCADE'), nullable=False)
    skip_key = db.Column(db.String(256), nullable=False, unique=True, comment='task_id + ip + open_ports 哈希键')
    ip = db.Column(db.String(45), nullable=False)
    open_ports = db.Column(db.String(128), nullable=False, comment='快速扫描阶段发现的端口，如 80,554')
    reason = db.Column(db.String(64), nullable=False, default='port_open_no_onvif')
...
class OnvifScanIpBlacklist(db.Model):
    """全局 IP 黑名单：扫描时跳过该地址（不探测、不注册、不更新），用于排除非目标设备。"""
    __tablename__ = 'onvif_scan_ip_blacklist'
