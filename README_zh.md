...
<ul style="font-size: 14px; line-height: 1.8; color: #444; margin: 10px 0;">
  ...
  <li><strong>移动端 APP 管理后台</strong>：基于 uni-app 3 构建跨端移动端管理后台，一套代码编译 H5、微信小程序与原生 App，与 PC 端（WEB）共用同一套后端 API（<code>/admin-api</code>），面向移动场景的运维与管理人员提供随时随地管控能力。核心 Tab 覆盖<strong>设备</strong>（直连摄像头、GB28181、NVR 多协议统一列表与通道浏览，设备详情内一键启动实时画面预览）、<strong>推流</strong>（推流转发任务创建、启停、集群节点状态与多路流地址查看）、<strong>算法</strong>（实时/抓拍算法任务列表、启停控制与检测统计）、<strong>告警</strong>（告警事件检索、抓拍图预览与告警录像点播回放）、<strong>模型</strong>（模型列表与部署状态）、<strong>推理</strong>（移动端图片推理工作台，选模型、传图片、看结果）、<strong>训练</strong>（训练任务进度监控与一键停止）及<strong>我的</strong>（个人资料、账号安全、多租户切换与应用设置）。H5 端集成 Jessibuca 低延时播放器，支持 FLV/HLS 实时流与告警录像点播；采用 OAuth2 双 Token 认证与 Pinia 状态持久化，登录态自动续期，让「云边端智能管控」真正触达手机与小程序</li>
</ul>
...
<td style="padding: 15px; border: 1px solid #e0e0e0; line-height: 1.8; color: #444;">
  <ul style="margin: 5px 0; padding-left: 20px;">
    <li><strong>跨端框架</strong>：基于 uni-app 3 + Vue 3 + TypeScript + Vite，一套代码编译 H5、微信小程序与原生 App</li>
    <li><strong>API 复用</strong>：与 WEB 模块共用 <code>/admin-api</code> 后端接口，OAuth2 双 Token 认证，支持多租户切换</li>
    <li><strong>设备管理</strong>：直连摄像头、GB28181、NVR 多协议统一列表，在线状态与通道浏览，设备详情内实时画面预览（H5 集成 Jessibuca 低延时播放器）</li>
