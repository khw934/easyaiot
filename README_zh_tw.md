...
<ul style="font-size: 14px; line-height: 1.8; color: #444; margin: 10px 0;">
  ...
  <li><strong>移動端 APP 管理後台</strong>：基於 uni-app 3 構建跨端移動端管理後台，一套代碼編譯 H5、微信小程序與原生 App，與 PC 端（WEB）共用同一套後端 API（<code>/admin-api</code>），面向移動場景的運維與管理人員提供隨時隨地管控能力。核心 Tab 覆蓋<strong>設備</strong>（直連攝像頭、GB28181、NVR 多協議統一列表與通道瀏覽，設備詳情內一鍵啟動實時畫面預覽）、<strong>推流</strong>（推流轉發任務創建、啟停、集群節點狀態與多路流地址查看）、<strong>算法</strong>（實時/抓拍算法任務列表、啟停控制與檢測統計）、<strong>告警</strong>（告警事件檢索、抓拍圖預覽與告警錄像點播回放）、<strong>模型</strong>（模型列表與部署狀態）、<strong>推理</strong>（移動端圖片推理工作台，選模型、傳圖片、看結果）、<strong>訓練</strong>（訓練任務進度監控與一鍵停止）及<strong>我的</strong>（個人資料、賬號安全、多租戶切換與應用設置）。H5 端集成 Jessibuca 低延時播放器，支持 FLV/HLS 實時流與告警錄像點播；採用 OAuth2 雙 Token 認證與 Pinia 狀態持久化，登錄態自動續期，讓「雲邊端智能管控」真正觸達手機與小程序</li>
</ul>
...
<td style="padding: 15px; border: 1px solid #e0e0e0; line-height: 1.8; color: #444;">
  <ul style="margin: 5px 0; padding-left: 20px;">
    <li><strong>跨端框架</strong>：基於 uni-app 3 + Vue 3 + TypeScript + Vite，一套代碼編譯 H5、微信小程序與原生 App</li>
    <li><strong>API 複用</strong>：與 WEB 模塊共用 <code>/admin-api</code> 後端接口，OAuth2 雙 Token 認證，支持多租戶切換</li>
    <li><strong>設備管理</strong>：直連攝像頭、GB28181、NVR 多協議統一列表，在線狀態與通道瀏覽，設備詳情內實時畫面預覽（H5 集成 Jessibuca 低延時播放器）</li>
...
<p style="font-size: 14px; line-height: 1.8; color: #2c3e50; font-weight: 500; margin: 20px 0; padding: 15px; background-color: #e8f4f8; border-left: 4px solid #3498db; border-radius: 4px;">
<strong>特別致謝</strong>：以上貢獻者在跨平台部署文檔與腳本、國標視訊能力落地與 AI 聯調驗證、多品牌攝影機直連發現與批量接入、天地圖空間視覺化完整落地、異構串流媒體叢集部署與排程架構、車牌識別演算法與完整程式碼落地、EasyAIoT-Edge 邊緣側端到端串聯、校園開發者社群組織與青年協作生態構建等不同方面推動了 EasyAIoT 的發展，他們的專業精神與無私奉獻值得我們學習與尊敬。再次向這些傑出的貢獻者表示最誠摯的感謝！🙏
</p>
